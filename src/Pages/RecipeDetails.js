...
const RecipeDetails = ({ history }) => {
  ...
  useEffect(() => {
    const getDetails = async () => {
      ...
      const URL = `https://www.the${isFood ? 'meal' : 'cocktail'
      }db.com/api/json/v1/1/lookup.php?i=${id.id}`;
    ...
    const getRecomendations = async () => {
      ...
      const response = await fetch(
        isFood ? DRINKS_RECOMENDATIONS : FOODS_RECOMENDATIONS,
      );
  ...
  const renderFoodDetails = () => {
    ...
    const renderRecomendations = () => {
      if (isFood) {
        return recomendations.drinks.map((e, index) => (
          <p key={ index } data-testid={ `${index}-recomendation-card` }>
            {e.strDrink}
          </p>
        ));
      }
      return recomendations.meals.map((e, index) => (
        <p key={ index } data-testid={ `${index}-recomendation-card` }>
          {e.strMeal}
        </p>
      ));
    ...
    return (
      <div>
        ...
        />
        {ingridientsAndMeasures().map((ingredient, index) => {
  ...
  return <div>{details && renderFoodDetails()}</div>;
};
